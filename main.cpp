...
void Block::saveBlock(Block block) {
    if (block.cur == 0) {
        cout << "Save wasn't done, current pointer doesn't exist." << endl;
        return;
    }
...
void saveBucketList(){
    // fo.open(FILENAME, ios::binary | ios::out);
    ...
    f.seekp(0, ios::beg);
    f.write((const char*)bucketList, sizeof(bucketList));
    f.close();
}
...
void loadBucketList(){
    ...
    fi.read((char*)bucketList, sizeof (bucketList));
    fi.close();
}
...
void insert(Student student) {
    ...
    int a = h(student.getId());
    size_t g = bucketList[a];
    if (g == 0) {
        Block b = Block::createBlock();
        bucketList[a] = b.cur;
        b.append(student);
        try {
            Block::saveBlock(b);
...
std::pair<Block, int> find(int val){
    int a = h(val);
    if (bucketList[a] == 0) return make_pair(Block(), -1);
    Block g;
    try {
        g = Block::getBlock(bucketList[a]);
    } catch (ios_base::failure f){
        cout << f.what() << endl;
...
int main() {
    ...
    f.close();
    loadBucketList();
    if (bucketList[M] == 0) bucketList[M] = (size_t)sizeof(bucketList);
    int switchValue;
    bool exit = false;
    while (!exit){
        cout << "--------------------------------------------------" << endl;
